document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleFile(file) {
    ...
    // Checking if imgly loaded from esm.sh
    if (typeof window.imglyRemoveBackground === 'undefined') {
      alert("Yapay Zeka (WASM) modeli henüz arkaplanda indiriliyor veya internet bağlantınız yavaş. Lütfen 3 saniye sonra tekrar deneyin.");
      return;
    }
      ...
      const config = {
        progress: (key, current, total) => {
          loadingStatus.innerHTML = `
            ...
            <small>(Görselin ağırlığına göre tarama yapılıyor)</small>
          `;
        }
      };
